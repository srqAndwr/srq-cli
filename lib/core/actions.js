...
const add3PageAction = async (name, dest) => {
    ...
    const routerResult = await renderFile('router.vue3.ejs', { name, lowerName: name.toLowerCase(),dest:dest.replace('src','@'),path:dest.split('src/views')[1] });
    // console.log(routerResult);
    //写入到目标文件
    const targetDir = path.resolve(dest, name.toLowerCase());
    const routerTargetDir = path.resolve('src/router' + (dest.split('src/views')[1] ? dest.split('src/views')[1]:''), name.toLowerCase());
    // console.log(dest.split('src/views')[1]);
    // console.log(targetDir);
    mkDir(targetDir);
